            ...
            <template #append>
              <v-dialog width="250">
                <template v-slot:activator="{ props }">
                  <v-btn v-bind="props" icon="mdi-delete" size="medium" variant="plain" color="red"></v-btn>
                </template>
                    ...
                    <template v-if="slotProps.exercises.length > 0">
                      <v-card-text>
                        <v-list>
                            ...
                            >
                              <v-list-item-title>
                                <span class="text-orange-darken-4">{{ exerciseNames[exercise.ExerciseId - 1] }}:</span> {{ exercise.Sets }} {{ exercise.Sets > 1 ? 'sets' : 'set' }} of {{ exercise.Reps }} {{ exercise.Reps > 1 ? 'reps' : 'rep' }} - {{ exercise.Weight }}kg
                              </v-list-item-title>
                            </v-list-item>
                        </v-list>
                      </v-card-text>
