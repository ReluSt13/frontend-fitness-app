  ...
  export default {
    ...
    methods: {
      ...
      async handleCreateFeedback(postId) {
        const result = await this.appStore.createFeedback({ postId: postId, value: true});
        if (result.isSuccess) {
      ...
      async handleDeleteFeedback(postId) {
        ...
        if (result.isSuccess) {
          const index = this.posts.findIndex(p => p.Id === postId);
          this.posts[index].Feedbacks = this.posts[index].Feedbacks.filter(f => f.UserId !== this.user.id);
        }
      },
      ...
      async handleDeleteComment(comment) {
        ...
        if (result.isSuccess) {
          ...
          this.posts[index].Comments = this.posts[index].Comments.filter(c => c.Id !== comment.Id);
        }
      },
      async handleEditComment(requestBody) {
        ...
        if (result.isSuccess) {
          const index = this.posts.findIndex(p => p.Id === requestBody.postId);
          const commentIndex = this.posts[index].Comments.findIndex(c => c.Id === requestBody.commentId);
